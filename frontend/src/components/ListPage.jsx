...
function StatusBadge({ status }) {
  ...
  if (status === "complete")
    return (
      <span className={`${base} ${l.statusBadgeComplete}`}>Completed</span>
    );
  if (status === "cancelled")
    return (
      <span className={`${base} ${l.statusBadgeCancelled}`}>Cancelled</span>
    );
  if (status === "confirmed")
    return (
      <span className={`${base} ${l.statusBadgeConfirmed}`}>Confirmed</span>
    );
  if (status === "rescheduled")
    return (
      <span className={`${base} ${l.statusBadgeRescheduled}`}>Rescheduled</span>
    );
  return <span className={`${base} ${l.statusBadgePending}`}>Pending</span>;
}
  ...
  if (appointment.status === "rescheduled") {
    return (
      <select
        ...
        className={`${l.statusSelect} ${
          terminal ? l.statusSelectDisabled : l.statusSelectEnabled
        }`}
      ...
      {!editing ? (
        <div className="flex justify-end">
          <button
            ...
            className={`${l.rescheduleButton} ${
              terminal ? l.rescheduleButtonDisabled : l.rescheduleButtonEnabled
            }`}
          >
            Reschedule
          </button>
    ...
    <div className={l.pageContainer}>
      <div className={l.contentWrapper}>
        <div className={l.headerContainer}>
          <div>
            <h1 className={l.headerTitle}>All Appointments</h1>
              ...
              <article key={a.id} className={l.appointmentCard}>
                ...
                <div className={l.dateTimeSection}>
                  <div className={l.dateTimeContainer}>
                    ...
                    <span className={l.dateText}>{formatDate(a.date)}</span>
