...
const ServiceCard = ({ service }) => {
  ...
  const src = service.imageUrl || service.image || service.imageSmall || PlaceholderImg;
  const srcSet =
    service.imageSrcSet ||
...
const ServicePage = ({ previewCount = 9999 }) => {
  ...
  return (
    <div className={sc.pageContainer}>
      <div className={sc.maxWidthContainer}>
        <header className={sc.header}>
          <h1 className={sc.title}>Our Diagnostic Servies</h1>
          <p className={sc.subtitle}>Safe, accurate & reliable testing</p>
        </header>

        {error && (
          <div className={sc.errorContainer}>
            <div className={sc.errorText}>
              <button className={sc.retryButton} onClick={loadServices}>
                Retry
              </button>
            </div>
          </div>
        ...
        {loading ? (
          <section className={sc.skeletonGrid}>
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className={sc.skeletonCard}>
                <div className={sc.skeletonImage}></div>
                <div className={sc.skeletonText1}></div>
                <div className={sc.skeletonText2}></div>
                <div className={sc.skeletonButton}></div>
              </div>
            ))}
          </section>
        ) : (
          <section className={sc.servicesGrid}>
            {shown.length > 0 ? (
              shown.map((s) => <ServiceCard key={s.id || s.name} service={s} />)
            ) : (
              <div className={sc.emptyState}>No service available</div>
            )}
          </section>
        )}
      </div>
