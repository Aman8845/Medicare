...
function dateTimeFromSlot(slot) {
  try {
    ...
    const [time, ampm = "AM"] = (slot.time || "12:00 AM").split(" ");
...
const AppointmentsPage = () => {
  ...
  const loadAppointments = useCallback(async () => {
    ...
    try {
      ...
      const url = `${API_BASE}/api/appointments?${params.toString()}`;
      const res = await fetch(url);
      ...
      <div className={pageStyles.maxWidthContainer}>
        <header className={pageStyles.headerContainer}>
          <div className={pageStyles.headerTitleSection}>
            ...
            <p className={pageStyles.headerSubtitle}>Manage all appointments</p>
          </div>
