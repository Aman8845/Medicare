  ...
  try {
    ...
    if (!clerkUserId)
      return res.status(401).json({
        success: false,
        message: "Authentication required to create service appointment.",
      });
...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    console.error("createServicePayment:", error);
    return res
      .status(500)
...
export const getServiceAppointments = async (req, res) => {
  try {
    ...
    return res.json({ success: true, appointments, total, page, limit });
  } catch (error) {
...
export const getServiceAppointmentById = async (req, res) => {
  try {
    const { id } = req.params;
    const list = await ServiceAppointment.findById(id).lean();
