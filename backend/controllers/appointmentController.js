...
export async function getAppointmentsByPatient(req, res) {
  try {
    ...
    console.log(
      "resolverCreatedBy (query or req.auth.userId):",
      resolvedCreatedBy,
    );
...
export async function confirmPayment(req, res) {
  try {
    ...
    try {
      ...
      return res.status(404).json({
        success: false,
        message: "Stripe session not found or invalid",
      });
    }
    if (!session)
      return res
...
// get Stats
export async function getStatsAppointment(req, res) {
  try {
